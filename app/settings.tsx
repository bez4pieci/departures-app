...
export default function Settings() {
  ...
  return (
    <View className="flex-1 bg-background p-4">
      {selectedStation && (
        <Text className="text-sm text-muted-foreground font-departure-mono mb-4">Current: {selectedStation.name}</Text>
      )}
      ...
      {error && <Text className="text-destructive mb-2 font-departure-mono">{error}</Text>}

      {isSearching && <Text className="text-muted-foreground mb-2 font-departure-mono">Searching...</Text>}

      <FlatList
        data={stations}
        keyExtractor={(item) => item.id}
        renderItem={({ item }) => (
          <TouchableOpacity onPress={() => selectStation(item)} className="p-3 border-b border-border active:bg-accent">
        ...
        ListEmptyComponent={
          searchQuery.trim() ? (
            <Text className="text-muted-foreground mt-4 text-center font-departure-mono">No stations found</Text>
          ) : null
        }
